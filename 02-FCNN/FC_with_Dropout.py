...
class Dropout:
	...
	def forward(self,Ain):
		self.Ain = Ain
		self.D = np.random.rand(*self.Ain.shape)
		self.D = (self.D < self.kp).astype(self.Ain.dtype)
		self.A = self.D*Ain
	def backwards(self,dA):
		self.dAin = self.D*dA
...
for e in range(EPOCHS):
	...
	for i in range(m//split):
		...
		l3.backwards(dY)
		d2.backwards(l3.dAin)
		l2.backwards(d2.dAin)
		d1.backwards(l2.dAin)
		l1.backwards(d1.dAin)
